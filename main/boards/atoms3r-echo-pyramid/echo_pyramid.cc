...
/** LED strip color: offset 0=B, 1=G, 2=R; offset 3 is register gap (no need to write). We send 4 bytes to keep stride. */
static void RgbToBgrBytes(uint32_t rgb, uint8_t out[4]) {
    out[0] = (rgb >> 0) & 0xFF;   /* B */
    out[1] = (rgb >> 8) & 0xFF;   /* G */
    out[2] = (rgb >> 16) & 0xFF;  /* R */
    /* out[3]: register gap, unused */
}
...
public:
    ...
    void setEffectMode(LightMode mode) {
        effect_mode_ = mode;
        if (mode == LightMode::RAINBOW) {
            setRgbStripBrightness(0, 30);
            setRgbStripBrightness(1, 30);
        }
    }
    ...
    bool readTouchStatus(uint8_t touch_num) {
        ...
        uint8_t buffer[1];
        i2c_master_transmit_receive(i2c_device_, &reg_addr, 1, buffer, 1, 300);
...
private:
    ...
    void setRgbColorInternal(uint8_t channel, uint8_t index, uint32_t color) {
        ...
        RgbToBgrBytes(color, write_buffer + 1);
        i2c_master_transmit(i2c_device_, write_buffer, sizeof(write_buffer), 100);
    }

    void setRgbChannelColorsInternal(uint8_t channel, const uint32_t colors[NUM_LEDS_PER_GROUP]) {
        uint8_t reg_addr;
        switch (channel) {
            case 0: reg_addr = REG_RGB_CH1_I1_COLOR; break;
            case 1: reg_addr = REG_RGB_CH2_I1_COLOR; break;
            case 2: reg_addr = REG_RGB_CH4_I1_COLOR; break;
            case 3: reg_addr = REG_RGB_CH3_I1_COLOR; break;
            default: return;
        }
        uint8_t write_buffer[1 + NUM_LEDS_PER_GROUP * 4];
        write_buffer[0] = reg_addr;
        const int order = (channel == 0 || channel == 1) ? -1 : 1;
        for (int i = 0; i < NUM_LEDS_PER_GROUP; i++) {
            int src = (order < 0) ? (NUM_LEDS_PER_GROUP - 1 - i) : i;
            RgbToBgrBytes(colors[src], &write_buffer[1 + i * 4]);
        }
        i2c_master_transmit(i2c_device_, write_buffer, sizeof(write_buffer), 100);
    }
        ...
        while (effect_running_) {
            switch (effect_mode_.load()) {
                ...
                case LightMode::RAINBOW: {
                    uint32_t ch_colors[NUM_RGB_CHANNELS][NUM_LEDS_PER_GROUP];
                    for (int c = 0; c < NUM_RGB_CHANNELS; c++) {
                        for (int i = 0; i < NUM_LEDS_PER_GROUP; i++) {
                            int led_pos = c * NUM_LEDS_PER_GROUP + i;
                            float led_hue = (hue + led_pos * 15.0f) / 360.0f;
                            ch_colors[c][i] = HsvToRgb(led_hue, 1.0f, 1.0f);
                        }
                        setRgbChannelColorsInternal(c, ch_colors[c]);
                    }
                    hue += 2.0f;
...
Si5351::Si5351(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr) {
    ...
    // Power down output drivers (regs 16-18)
    {
        uint8_t write_buffer[] = { 16, 0x80, 0x80, 0x80 };
        ESP_ERROR_CHECK(i2c_master_transmit(i2c_device_, write_buffer, sizeof(write_buffer), 100));
    }
    ...
    // Multisynth NA Parameters (regs 26-33)
    {
        uint8_t write_buffer[] = { 26, 0xFF, 0xFD, 0x00, 0x09, 0x26, 0xF7, 0x4F, 0x72 };
        ESP_ERROR_CHECK(i2c_master_transmit(i2c_device_, write_buffer, sizeof(write_buffer), 100));
    }
    ESP_LOGI(TAG, "Si5351 Registers 26-33 (Multisynth NA) configured");

    // Multisynth1 Parameters (regs 50-57)
    {
        uint8_t write_buffer[] = { 50, 0x00, 0x01, 0x00, 0x2F, 0x00, 0x00, 0x00, 0x00 };
        ESP_ERROR_CHECK(i2c_master_transmit(i2c_device_, write_buffer, sizeof(write_buffer), 100));
    }
